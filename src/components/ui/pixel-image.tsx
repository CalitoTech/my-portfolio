  ...
  return (
    <div className={cn("relative h-full w-full select-none", className)}>
      {isFinished ? (
        <img
          src={src}
          alt="Full Pixel Image"
          className={cn(
            "w-full h-full object-cover rounded-[inherit] transition-all duration-1000",
            grayscaleAnimation && (showColor ? "grayscale-0" : "grayscale")
          )}
          style={{
            filter: grayscaleAnimation
              ? `grayscale(${showColor ? 0 : 1})`
              : "none",
          }}
        />
      ) : (
        pieces.map((piece, index) => (
          ...
          >
            <img
              ...
              className={cn(
                "z-1 rounded-[inherit] object-cover w-full h-full",
                grayscaleAnimation && (showColor ? "grayscale-0" : "grayscale")
              )}
              style={{
